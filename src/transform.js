...
function transform(input) {
  ...
  const hasPayload =
    Array.isArray(payload.departures) || Array.isArray(payload.arrivals)
  ...
  const services = list.map((d) => {
    ...
    if (d.plannedDateTime && d.actualDateTime) {
      delay = Math.round(
        (Date.parse(d.actualDateTime) - Date.parse(d.plannedDateTime)) / 60000,
      )
    }
